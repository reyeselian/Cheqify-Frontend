...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .steps { display: flex; align-items: center; justify-content: center; margin-bottom: 1rem; }
  .step-dot {
  ...
  .step-dot.active { background: #0ea5e9; border-color: #0ea5e9; color: #fff; box-shadow: 0 0 0 3px rgba(14,165,233,0.15); }
  ...
  .plan-card {
    ...
    padding: 1.2rem 1.1rem; cursor: pointer;
    transition: all 0.28s ease; overflow: hidden;
    box-shadow: 0 1px 4px rgba(0,0,0,0.04);
  }
  .plan-card:hover { border-color: #cbd5e1; box-shadow: 0 6px 20px rgba(0,0,0,0.08); transform: translateY(-2px); }
  .plan-card.selected {
    border-color: var(--plan-color);
    box-shadow: 0 0 0 1px var(--plan-color), 0 10px 28px rgba(0,0,0,0.09);
  ...
  .trial-tag {
    ...
    background: color-mix(in srgb, var(--plan-color) 10%, transparent);
    border-radius: 6px; padding: 2px 7px; margin-bottom: 0.5rem; display: inline-block;
  }
  .plan-divider { height: 1px; background: #f1f5f9; margin-bottom: 0.7rem; }
  .plan-features { list-style: none; display: flex; flex-direction: column; gap: 0.4rem; }
  .plan-features li { font-size: 0.82rem; color: #475569; display: flex; align-items: center; gap: 7px; }
  .plan-features li .check {
    ...
    display: flex; align-items: center; justify-content: center;
    font-size: 0.55rem; color: var(--plan-color); font-weight: 700;
  }
  .select-indicator {
  ...
  .skeleton-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.9rem; margin-bottom: 1rem; }
  @media (max-width: 700px) { .skeleton-grid { grid-template-columns: 1fr; } }
  .skeleton-card { background: #fff; border: 2px solid #e2e8f0; border-radius: 18px; padding: 1.2rem 1.1rem; height: 300px; }
  .skel {
  ...
  .selected-plan-chip {
    ...
    font-size: 0.8rem; color: #475569; margin-bottom: 1.1rem;
  ...
  .msg-alert { border-radius: 9px; padding: 0.65rem 0.9rem; font-size: 0.85rem; margin-bottom: 1rem; border: 1px solid; }
  ...
  .login-link { text-align: center; margin-top: 1rem; font-size: 0.84rem; color: #94a3b8; }
...
const Register: React.FC = () => {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/plans`)
      ...
      .catch(() => setPlansError("No se pudieron cargar los planes. Intenta de nuevo."))
      .finally(() => setPlansLoading(false));
  }, []);

  const handleNext = () => { if (!selectedPlan) return; setStep(2); };
  const handleBack = () => { setStep(1); setErrorMsg(""); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post(`${API_URL}/auth/register`, {
        ...
        plan: selectedPlan.type, planId: selectedPlan._id,
      });
  ...
  const handleResend = async () => {
    ...
    setResendMsg("");
    try {
      const { data } = await axios.post(`${API_URL}/auth/resend-verification`, { email: registeredEmail });
      setResendMsg(data.message);
    } catch { setResendMsg("Error al reenviar. Intenta de nuevo."); }
    finally { setResending(false); }
  };

  const formatPrice = (plan: IPlanAPI) => plan.type === "trial" ? "Gratis" : `$${plan.price}`;
        ...
        <div className="ve-root">
          <div style={{ width: "100%", maxWidth: "460px" }}>
            ...
            <div className="ve-card">
              ...
              <p className="ve-msg">Haz clic en el enlace del correo para activar tu cuenta.<br />Revisa también tu carpeta de <strong>spam</strong>.</p>
              ...
              <div className="ve-login-link">¿Ya verificaste? <a href="/login">Iniciar sesión</a></div>
            </div>
          </div>
        </div>
      </>
        ...
        <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          {step === 1 && (
            <div className="plans-wrapper">
                ...
                <div className="skeleton-grid">
                  {[1,2,3].map((i) => (
                    <div key={i} className="skeleton-card">
                      ...
                      <div className="skel" style={{ width: "40%", height: 32, marginTop: 6 }} />
                      <div className="skel" style={{ width: "100%", height: 1, marginTop: 12 }} />
                      {[80,90,75].map((w,j) => <div key={j} className="skel" style={{ width: `${w}%`, height: 11 }} />)}
                    </div>
                  ))}
                </div>
              )}
              ...
              {!plansLoading && !plansError && (
                <div className="plans-grid">
                  {plans.map((plan) => {
                    ...
                    return (
                      <div key={plan._id} className={`plan-card ${isSelected ? "selected" : ""}`}
                      ...
                      >
                        ...
                        <div className="select-indicator">{isSelected ? "✓ Seleccionado" : "Seleccionar"}</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <button className="btn-next" onClick={handleNext} disabled={!selectedPlan || plansLoading}>
                Continuar con el registro →
              </button>
              <div className="login-link">¿Ya tienes cuenta? <a href="/login">Inicia sesión</a></div>
            </div>
          )}
          ...
          {step === 2 && selectedPlan && (
            <div className="form-wrapper">
              ...
              <div className="form-card">
                ...
                <div className="selected-plan-chip" onClick={handleBack} title="Cambiar plan">
                ...
                <form onSubmit={handleSubmit}>
                  <div className="field-group">
                    ...
                    <div className="field-input-wrap">
                      <input className="field-input" type="text" placeholder="Ej: Grupo Fénix SRL"
                  ...
                  <div className="field-group">
                    ...
                    <div className="field-input-wrap">
                      <input className="field-input" type="email" placeholder="correo@empresa.com"
